...
| Member tasks | Create ledger tasks with acceptance criteria, send them to the member, steer it, and follow available worker reports through To do, In progress, Blocked, Review and Done; Done requires acceptance. Drafts survive route changes. Automatic worker progress is unavailable for members with private memory | Crew Members → right panel → **Tasks** | `pages/members/MemberTasks.tsx`, `pages/members/MembersPage.tsx`, `types/memberWork.ts`, `store/dashboardSlice.ts` | `handlers/member_work.py`, `work_ledger.py` | `GET,POST /api/members/{slug}/work`, `POST /api/chat` (steering) |
