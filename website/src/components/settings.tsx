    ...
    <SettingsField label={label} description={description} hint={hint} configKey={configKey} controlId={controlId}>
      {multiline ? (
        <textarea
          ...
          className={`w-full rounded border border-border bg-bg px-2 py-1 text-sm text-text focus-visible:border-accent focus:outline-none resize-y flex-none ${disabled ? 'opacity-40 cursor-not-allowed' : ''}`}
        />
      ) : (
        <Input
          id={controlId}
          ...
          className={`flex-none ${disabled ? 'opacity-40 cursor-not-allowed' : ''}`}
        />
      )}
    </SettingsField>
  )
