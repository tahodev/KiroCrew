 ...
 *  window's persisted watermarks against the shared unread list: a MESSAGE
 *  watermark whose badge a sibling window cleared while this tab was
 *  unloaded is an orphan — invisible in the UI, yet it would shield its slot
 *  from every future remote clear — so those are dropped and the pruned map
 *  written back. A MANUAL_UNREAD sentinel is the opposite case: live-window
 *  semantics say no other window's read may clear the deliberate reminder,
 *  and a reload must not demote it either — the sentinel survives, and
 *  restoreUnreadBadges() below re-seeds its badge into this window's list
 *  (in-memory union only; the shared key is not written, mirroring how a
 *  live window shows the reminder without publishing sibling state). */
export const restoreUnreadSince = (): Record<string, string> => {
  try {
    ...
    const live = Object.fromEntries(Object.entries(since).filter(
      ([k, v]) => v === MANUAL_UNREAD || badges.includes(k)))
