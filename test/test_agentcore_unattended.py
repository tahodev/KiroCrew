...
async def test_workload_user_without_vault_retracts_gateway() -> None:
    ...
    # Retract is the absence of a session entry: the Gateway is never written
    # into the agent file, so nothing needs a placeholder to outrank it.
    assert injected == []
    events = [
        ...
        json.dumps(
            {
                ...
                "url": "https://gw.gateway.bedrock-agentcore.us-east-1.amazonaws.com/mcp",
            }
        ),
        encoding="utf-8",
    )
